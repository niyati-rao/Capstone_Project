...
Blum-Kalai-Wasserman (BKW) Algorithm for recovering the secret vector.
...
=
𝑏
A⋅s+e=b where A is a binary matrix, s is the secret vector, and b is the noisy output vector. The vector e represents the error introduced to b with some error rate.
...
Problem Generation: The LPN problem instance is generated by creating a random binary secret vector s and a binary matrix A. A random noise vector is then added to b, creating a noisy system of equations.
